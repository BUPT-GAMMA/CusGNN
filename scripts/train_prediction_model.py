...
import sys
import torch
from models.component_prediction.experts_gat_predictor import GatSPredictor
    ...
    def objective(trial):
        with mlflow.start_run(experiment_id=exp_id) as run:  # automatically terminates run
            ...
            if model_type == 'gcn':
                ...
                # }
                num_heads = 8
                model = GatSPredictor(node_feature_dim=embedding_width, hidden_dim=32, num_heads=num_heads,
                                     num_hidden_layers=3, num_classes=vocabulary_size,
                                     feat_drop=0.3, attn_drop=0.0, residual=True)
            else:
